...
async function CrearBaseSiNoExiste(){
    ...
    if(!existe){
        ...
        await db.run(
            ...
            "(1, 'admin', '123', 'admin'), (2, 'juan', '123', 'member'), (3, 'manu', '123', 'member');"
        );
    }

    existe = false;
    res = await db.get(
        "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' AND name = 'articulos_familias'",
        []
    );
    if (res.contar > 0) existe = true;
    if(!existe){
        await db.run(
            "CREATE TABLE articulos_familias (IdArticuloFamilia INTEGER PRIMARY KEY AUTOINCREMENT, Nombre TEXT NOT NULL UNIQUE);"
        );
        console.log("Tabla articulos_familias creada!");
        await db.run(
            "INSERT INTO articulos_familias VALUES" +
            "(1,'ACCESORIOS'),(2,'AUDIO'),(3,'CELULARES'),(4,'CUIDADO PERSONAL'),(5,'DVD'),(6,'FOTOGRAFIA'),(7,'FRIO-CALOR'),(8,'GPS'),(9,'INFORMATICA'),(10,'LED - LCD');"
        );
    }
    ...
    if(!existe){
        await db.run(
            ...
            "FechaAlta TEXT, " +
            "Activo BOOLEAN, " +
            "FOREIGN KEY (IdArticuloFamilia) REFERENCES articulos_familias (IdArticuloFamilia));"
        ...
        await db.run(
            ...
            "(43, 'LED 46\" PHILIPS SMART TV 46PFL4508G/77', 13499.00, '0779813042264', 10, 541, '2017-01-09', 1), " +
            "(44, 'LED 46\" SAMSUNG SMART TV 3D UN46F7500', 16999.00, '0779808337380', 10, 298, '2017-02-02', 1), " +
            "(45, 'LED 50\" BGH FHD BLE5014D', 18999.00, '0779688540146', 10, 799, '2017-02-08', 1), " +
            "(46, 'LED 50\" LG FHD 50LN5400', 16999.00, '0779808339498', 10, 217, '2017-01-05', 1), " +
            "(47, 'LED 55\" LG UHD 4K 55UB8300', 29999.00, '0779808339701', 10, 389, '2017-01-27', 1), " +
            "(48, 'LED 55\" PHILIPS SMART 3D 55PFL8008G/77', 31999.00, '0779813042257', 10, 278, '2017-01-20', 1), " +
            "(49, 'LED 60\" SONY UHD 4K 60X850B', 44999.00, '0490552491882', 10, 137, '2017-01-15', 1), " +
            "(50, 'LED 65\" SAMSUNG UHD 4K 65HU9000', 49999.00, '0779808337384', 10, 695, '2017-01-17', 1), " +
            "(51, 'LED 70\" SHARP UHD 4K LC-70UD1U', 69999.00, '0740003745104', 10, 587, '2017-01-21', 1), " +
            "(52, 'LED 75\" SONY UHD 4K 75X940D', 79999.00, '0490552491835', 10, 223, '2017-01-28', 1), " +
            "(53, 'OLED 55\" LG UHD 4K 55EF9500', 99999.00, '0779808339017', 10, 345, '2017-01-12', 1), " +
            "(54, 'OLED 65\" LG UHD 4K 65EG9600', 149999.00, '0779808339031', 10, 478, '2017-01-14', 1), " +
            "(55, 'OLED 77\" LG UHD 4K 77EG9700', 199999.00, '0779808339038', 10, 651, '2017-02-03', 1);" +
